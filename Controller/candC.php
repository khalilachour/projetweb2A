...
require_once 'C:\xampp\htdocs\projet\Model\candM.php';
include_once 'C:/xampp/htdocs/projet/Controller/compC.php';
...
class CandidatureController {
    ...
    public function addCandidature($id_offre, $id_user, $date, $cv, $lettre){
        $db = Config::getConnexion();
        try {
            $sql = "INSERT INTO candidatures (id_offre, id_user, date, cv, lettre) VALUES (:id_offre, :id_user, :date, :cv, :lettre)";
            $stmt = $db->prepare($sql);
   ...
   /* public function deleteCandidature($id){
    ...
    public function updateCandidature($id, $id_offre, $id_user, $date, $cv, $lettre){
        $db = Config::getConnexion();
        try {
            $sql = "UPDATE candidatures SET id_offre = :id_offre, id_user = :id_user, date = :date, cv = :cv, lettre = :lettre WHERE id = :id";
            $stmt = $db->prepare($sql);
            $stmt->bindParam(':id_offre', $id_offre);
            $stmt->bindParam(':id_user', $id_user);
            $stmt->bindParam(':date', $date);
    ...
    public function getOffreDetailsByCandidatureId($id_candidature) {
        $db = Config::getConnexion();
        try {
            $sql = "SELECT offres.id, offres.titrePoste, offres.lieu FROM offres 
    ...
    public function listCandidaturesByUser($user_id) {
        $db = Config::getConnexion();
        try {
            // Préparez votre requête SQL pour sélectionner les candidatures filtrées par date
            $sql = "SELECT * FROM candidatures WHERE id_user = :user_id";
            $stmt = $db->prepare($sql);
            // Liez le paramètre de date
    ...
    // Méthode pour récupérer les candidatures d'un utilisateur spécifique
    public function listCandidaturesByOffre($offre_id) {
        $db = Config::getConnexion();
        try {
            // Préparez votre requête SQL pour sélectionner les candidatures filtrées par date
            $sql = "SELECT * FROM candidatures WHERE id_offre = :offre_id";
            $stmt = $db->prepare($sql);
            // Liez le paramètre de date
