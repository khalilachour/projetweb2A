...
class FeedbackC {
    ...
    public function createFeedback() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            header('Location: index1.php?action=viewFeedback&eventId=' . $_POST['event_id']);
            exit();
        }
    ...
    public function reportFeedback($feedbackId) {
        try {
            ...
            $stmt = $this->pdo->prepare("UPDATE feedback SET feedback_text = ? WHERE feedback_id = ?");
            $stmt->execute([$censoredText, $feedbackId]);
